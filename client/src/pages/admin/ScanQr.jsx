...
export default function ScanQr() {
  ...
  /*
   * IMPORTANT:
   * Serialized damaged assets have their own conditionNote.
   * The previous validation did not check these.
   */
  const damagedSerializedAsset = returnAssets.find(
    (entry) =>
      entry.condition === "damaged" &&
      String(entry.conditionNote || "").trim().length < 5
  );
  ...
  /*
   * Validate missing serialized assets.
   */
  const invalidMissingAsset = missingAssets.find(
    (entry) =>
      String(entry.reason || "").trim().length < 5
  );
  ...
  const normalAccounted = returnForm.items.reduce(
    (sum, item) => {
      ...
      /*
       * Serialized quantities are calculated from
       * scanned/missing assets below.
       */
      if (requested?.trackingType === "serialized") {
        return sum;
      }
  ...
  /*
   * Count serialized assets.
   */
  const serializedAccounted =
    returnAssets.length + missingAssets.length;
  ...
  try {
    /*
     * Build counts for serialized assets.
     */
    const serializedCounts = new Map();

    for (const asset of returnAssets) {
      const key = String(asset.asset.inventoryId);
    ...
    const submission = {
      ...
      items: returnForm.items.map((item) => {
        ...
        /*
         * Bulk item:
         * keep manually entered quantities.
         */
        if (requested?.trackingType !== "serialized") {
          return {
            ...item,
        ...
        /*
         * Serialized item:
         * calculate quantities from scanned assets.
         */
        const counts =
        ...
        return {
          ...item,
          goodQuantity: counts.good || 0,
          damagedQuantity: counts.damaged || 0,
          missingQuantity: counts.missing || 0,
          conditionNote:
      ...
      missingAssets: missingAssets.map(
        ({ assetId, reason }) => ({
          assetId,
    ...
    /*
     * Debug:
     * This confirms that the submit handler reached
     * the actual API call.
     */
    console.log(
  ...
  return <div className="qr-page scan-page">
    ...
    {result?.mode === "return" && <div className="transaction-result-overlay"><section className="claim-card return-scan-card transaction-result-dialog" role="dialog" aria-modal="true" aria-label="Process borrowing return"><div className="result-dialog-top"><span className="result-status return"><FaCheckCircle /> Borrower verified</span><button type="button" className="result-close" onClick={clearResult} aria-label="Close return"><FaTimes /></button></div>
      ...
      {result.requests.length > 1 && <fieldset className="return-request-picker"><legend><span className="scanner-step">Step 2</span> Choose the transaction being returned</legend><p>This borrower has {result.requests.length} active borrowings. Confirm the request ID and items before continuing.</p><div>{result.requests.map((request) => { const selected = String(returnRequest?.id) === String(request.id); const outstanding = request.items.filter((item) => Number(item.outstandingQuantity) > 0); return <button type="button" className={selected ? "selected" : ""} aria-pressed={selected} key={request.id} onClick={() => selectReturnRequest(request)}><span><strong>BR-{String(request.id).padStart(3, "0")}</strong><small>{String(request.borrowDate).slice(0, 10)} to {String(request.returnDate).slice(0, 10)}</small></span><span className="return-request-items">{outstanding.map((item) => `${item.name} ×${item.outstandingQuantity}`).join(", ")}</span><b>{selected ? "Selected" : "Choose"}</b></button>; })}</div></fieldset>}
      {returnRequest && returnForm && <form className="qr-return-form" onSubmit={submitReturn} noValidate>
  ...
  />
)}<button type="button" onClick={() => setReturnAssets((current) => current.filter((_, index) => index !== assetIndex))}>Remove</button></div>)}</div>}
