...
export default function Calendar() {
  ...
  const loadCalendar = useCallback(async () => {
    const [eventsResult, roomsResult] = await Promise.all([
      ...
      supabase.from("laboratory_rooms").select("*").eq("is_active", true).order("name"),
    ]);
    const queryError = eventsResult.error || roomsResult.error;
    if (queryError) {
      setError(queryError.message);
      return;
    }
    setError("");
    const roomNames = new Map((roomsResult.data || []).map((room) => [String(room.id), room.name]));
    setRooms(roomsResult.data || []);
    setEvents((eventsResult.data || []).map((event) => mapEvent(event, roomNames)));
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "laboratory_rooms" }, loadCalendar)
      .subscribe();
    return () => {
  ...
  const saveEvent = async (form) => {
    ...
    const response = await fetch(
      editingEvent ? `${apiUrl}/api/calendar/events/${editingEvent.id}` : `${apiUrl}/api/calendar/events`,
      {
    ...
    if (!response.ok) throw new Error(result.reasons?.[0] || result.message || "Unable to save event.");
    setOpenModal(false);
    setEditingEvent(null);
    await loadCalendar();
  };
  ...
  const deleteEvent = async (event) => {
    const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";
    const response = await fetch(`${apiUrl}/api/calendar/events/${event.id}`, { method: "DELETE" });
    if (!response.ok) {
      const result = await response.json();
  ...
  return (
    <div className="calendar-page">
      ...
      <div className="calendar-layout">
        ...
        {calendarView === "Week" && <WeekView events={events} selectedDate={selectedDate} />}
        {calendarView === "Day" && <DayView events={events} selectedDate={selectedDate} />}
        {calendarView === "Schedule" && <ScheduleView events={events} />}
      </div>
      ...
      />
      <DeleteModal
        key={deletingEvent?.id ?? "delete-event"}
