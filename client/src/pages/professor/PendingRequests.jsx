  ...
  return <div className="professor-requests-page">
    ...
    <div className="professor-request-list">
      ...
      {filtered.map((request) => <article className="professor-request-card" key={request.id}>
        <div className="professor-request-main"><div className="professor-request-student"><div className="professor-student-avatar">{request.studentName?.charAt(0)}</div><div><h3>{request.studentName}</h3><span>{request.studentId} · {request.sectionName || "Section not assigned"}</span></div></div><p>{request.purpose}</p><div className="professor-request-meta"><span>{request.departmentName || "Department not assigned"}</span><span>{formatDate(request.borrowDate)} – {formatDate(request.returnDate)}</span><span>{(request.items || []).reduce((sum, item) => sum + Number(item.quantity), 0)} unit(s)</span></div></div>
