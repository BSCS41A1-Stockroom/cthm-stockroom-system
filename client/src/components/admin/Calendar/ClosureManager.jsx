...
const EMPTY = { title: "", startDate: "", endDate: "", departmentId: "", sourceUrl: "", sourceKind: "school_announcement" };
...
export default function ClosureManager({ onChange }) {
  ...
  const load = useCallback(async () => {
    try {
      const [closureResponse, optionResponse] = await Promise.all([
        authenticatedFetch("/api/calendar-closures"),
        authenticatedFetch("/api/borrowings/assignment-options"),
      ]);
      const [closureBody, optionBody] = await Promise.all([closureResponse.json(), optionResponse.json()]);
      if (!closureResponse.ok || !optionResponse.ok) throw new Error(closureBody.message || optionBody.message || "Unable to load closure settings.");
  ...
  const suggest = async () => {
    ...
    try {
      ...
      setSuggestion(body);
      if (body.dates.length) setForm((current) => ({ ...current, title: current.title || "Class suspension", startDate: body.dates[0], endDate: body.dates.at(-1) }));
    } catch (reason) { setError(reason.message); }
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setForm(EMPTY); setCaption(""); setSuggestion(null);
  ...
  return <section className="closure-manager">
    ...
    {open && <div className="closure-manager-panel">
      <h3>Confirm a school closure</h3>
      <p>Paste an official announcement to suggest dates. Review the original post before blocking schedules.</p>
      <label>Announcement caption<textarea value={caption} maxLength={10000} onChange={(event) => setCaption(event.target.value)} rows={3} /></label>
      ...
      <form onSubmit={save} className="closure-form">
        <label>Closure type<select value={form.sourceKind} onChange={(event) => setForm({ ...form, sourceKind: event.target.value })}><option value="school_announcement">School announcement</option><option value="official_holiday">Official holiday</option></select></label>
        ...
        <label>Official post link (optional)<input type="url" value={form.sourceUrl} onChange={(event) => setForm({ ...form, sourceUrl: event.target.value })} placeholder="https://facebook.com/..." /></label>
        <button type="submit" disabled={busy}>Confirm and block dates</button>
      </form>
