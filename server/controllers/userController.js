...
function invitationRedirectUrl(
    ...
    const origin = String(clientUrl ?? "")
        .split(",")[0]
        .trim();

    if (!origin) {
        throw new Error(
            "CLIENT_URL is required for account invitations."
        );
    }
    ...
    if (
        ...
        throw new Error(
            "CLIENT_URL must use HTTPS for account invitations."
        );
    }
...
function getAdminClient() {
    ...
    if (!url || !key) {
        const error = new Error(
            "User invitations are not configured on the server."
        );

        error.code =
            "USER_ADMIN_NOT_CONFIGURED";
    ...
    if (
        ...
        const error = new Error(
            "SUPABASE_URL is invalid on the server."
        );
...
function userErrors(
    ...
    if (
        user.role === "student" &&
        (!user.studentId ||
            user.studentId.length > 100)
    ) {
        errors.push(
            "A student ID is required for Student accounts."
        );
    }
...
async function listUsers(req, res, next) {
    ...
    try {
        const result = await pool.query(
            ...
            LEFT JOIN public.academic_departments department
                ON department.id =
                    profile.department_id

            LEFT JOIN public.academic_sections section
                ON section.id =
                    profile.section_id
    ...
    try {
        ...
        if (
            ...
            return res.status(422).json({
                ...
                message:
                    "Select an active section belonging to the selected department.",
            });
        }
        ...
        if (
            ...
            return res.status(503).json({
                error:
                    "USER_ADMIN_NOT_CONFIGURED",
                message:
                    "User invitations are unavailable because the server's Supabase Admin credentials are invalid. Ask an administrator to update SUPABASE_SECRET_KEY.",
            });
        }
        ...
        if (
            ...
            return res.status(409).json({
                error:
                    "STUDENT_ID_IN_USE",
                message:
                    "That student ID is already assigned to another account.",
            });
        }
            ...
            await client.query(
                ...
                SET
                    ...
                    qr_version =
                        qr_version + 1,
                    qr_revoked_at = now(),
                    qr_revocation_reason =
                        'Account role changed from Student',
                    updated_at = now()
