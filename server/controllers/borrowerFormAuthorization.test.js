...
test("places verified staff and approved department-head signature snapshots on their form lines", () => {
  const signature = Buffer.from([137,80,78,71,13,10,26,10,0]);
  const form = generateBorrowerForm({ items: [{ description:"Pan",quantity:1 }],
    ...
    approvedName:"Department Head",approvedAt:"September 20, 2026",approvedSignature:signature,approvedSignatureMime:"image/png" });
  const zip = new PizZip(form);
  ...
  assert.match(xml,/Department Head/);
