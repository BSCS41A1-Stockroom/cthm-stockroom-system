...
const {
  loadInventoryCommitment,
  usableInventoryQuantity,
} = require("../utils/inventoryCommitments");

const INVENTORY_NUMBER_FIELDS = [
  "quantity",
  "additional_qty",
  "replaces",
  "missing",
...
function inventoryErrors(item) {
  ...
  if (
    ...
    errors.push(
      "Item name must contain between 1 and 200 characters."
    );
  }

  if (
    item.purchase_date != null &&
    !isValidDate(item.purchase_date)
  ) {
    errors.push(
  ...
  if (item.remarks.length > 1000) {
    errors.push(
      "Remarks cannot exceed 1000 characters."
    );
  }

  if (item.image_url.length > 2000) {
    errors.push(
  ...
  for (const field of INVENTORY_NUMBER_FIELDS) {
    if (
      ...
      item[field] < 0 ||
      item[field] > POSTGRES_INTEGER_MAX
    ) {
      errors.push(
        `${field} must be a non-negative database-safe whole number.`
      );
    }
  }
  ...
  if (
    ...
    errors.push(
      "Inventory deductions cannot exceed the total physical quantity."
    );
  }
...
async function saveInventory(req, res, next) {
  ...
  const inventoryId =
    req.params.inventoryId ?? null;

  /* =======================================================
     SERIALIZED INVENTORY VALIDATION
  ======================================================= */

  if (
    inventoryId == null &&
    item.tracking_type === "serialized" &&
    INVENTORY_NUMBER_FIELDS.some(
      (field) =>
    ...
    return res.status(422).json({
      error: "INVALID_SERIALIZED_INVENTORY",
      reasons: [
        "Serialized inventory starts at zero; add each physical asset after saving the classification.",
      ],
    });
  }
  ...
  try {
    ...
    if (inventoryId != null) {
      ...
      if (
        previous.tracking_type === "serialized" &&
        INVENTORY_NUMBER_FIELDS.some(
          (field) =>
            field !== "low_stock_threshold" &&
            Number(previous[field] ?? 0) !== item[field]
        )
      ) {
        await client.query("ROLLBACK");

        return res.status(409).json({
          error: "SERIALIZED_COUNTERS_MANAGED",
          message:
            "Serialized quantities and loss counters are managed through the physical asset records.",
        });
      }
      ...
      if (
        previous.tracking_type !== item.tracking_type
      ) {
        ...
        const hasStock =
          INVENTORY_NUMBER_FIELDS.some(
            (field) =>
              field !== "low_stock_threshold" &&
              Number(previous[field] ?? 0) !== 0
          );

        if (
          hasStock ||
          Number(assetCount.rows[0].count) > 0
        ) {
          await client.query("ROLLBACK");

          return res.status(409).json({
            error: "TRACKING_TYPE_LOCKED",
            message:
              "Tracking type can only be changed while the classification has no stock, assets, losses, or active counters.",
          });
        }
      }

      /* ===================================================
         BORROWING COMMITMENT VALIDATION
      =================================================== */
      ...
      if (!commitment.valid) {
        ...
        return res.status(409).json({
          ...
          message:
            "Existing borrowing commitments are inconsistent. Inventory was not changed.",
        });
      }

      if (
        usableInventoryQuantity(item) <
        commitment.requiredCapacity
      ) {
        await client.query("ROLLBACK");

        return res.status(409).json({
          error: "INVENTORY_COMMITMENT_CONFLICT",
          message: `Usable quantity cannot be lower than the ${commitment.requiredCapacity} unit(s) required by active borrowings.`,
        });
      }
    }
    ...
    return res
      .status(inventoryId == null ? 201 : 200)
      .json({
        item: result.rows[0],
      });
    ...
    if (error.code === "23505") {
      return res.status(409).json({
        ...
        message:
          "An inventory item with the same unique value already exists.",
      });
    }
...
async function deleteInventory(req, res, next) {
  ...
  try {
    ...
    if (references.rowCount) {
      ...
      return res.status(409).json({
        error: "INVENTORY_HAS_HISTORY",
        message:
          "Items with borrowing history cannot be deleted. Mark the item unavailable instead.",
      });
    ...
    if (assets.rowCount) {
      ...
      return res.status(409).json({
        error: "INVENTORY_HAS_ASSETS",
        message:
          "A serialized classification with physical asset records cannot be deleted. Retire its assets instead.",
      });
    ...
    await writeAuditLog(client, req.user, {
      ...
      entityId: req.params.inventoryId,
...
async function listUnavailability(
  req,
  res,
  next
) {
...
async function saveUnavailability(
  ...
  const period =
    normalizeUnavailability(req.body);

  const errors =
    unavailabilityErrors(period);

  if (errors.length) {

    return res.status(422).json({
      error:
        "INVALID_UNAVAILABILITY_PERIOD",
  ...
  try {
    ...
    if (periodId) {
      ...
      if (
        ...
        return res.status(404).json({
          ...
          message:
            "Unavailability period was not found.",
        });
    ...
    if (overlapResult.rowCount > 0) {
      ...
      return res.status(409).json({
        error:
          "UNAVAILABILITY_PERIOD_OVERLAP",

        message:
          "This item already has an unavailable period that overlaps those dates.",
      });
    ...
    const activeBorrowingResult =
      await client.query(
        `
        SELECT requests.id
        FROM borrow_requests requests
        JOIN borrow_request_items items
    ...
    if (
      activeBorrowingResult.rowCount > 0
    ) {
      ...
      return res.status(409).json({
        error:
          "ACTIVE_BORROWING_DURING_UNAVAILABILITY",

        message:
          "An active borrowing request already uses this item during those dates. Resolve that request first.",
      });

    }

    /* =====================================================
       INSERT / UPDATE PERIOD
    ===================================================== */
...
async function deleteUnavailability(
  ...
  try {
    ...
    await client.query(
      `
      DELETE FROM inventory_unavailability
      WHERE id = $1
      `,
      [req.params.periodId]
    );
